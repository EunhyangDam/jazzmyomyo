...
function Sub083MmGrade() {
  ...
  const loadFromServer = async () => {
    try {
      const { data } = await axios.get("/jazzmyomyo/grade_table_select.php", {
        timeout: 10000,
      });
  ...
  const onSave = async () => {
    try {
      const cleaned = grades.filter(
        (g) => (g.name || g.condition || g.benefit).trim?.() !== ""
      );
      ...
      const res = await axios.post(
        "/jazzmyomyo/grade_table_update.php",
        formData
      );
        ...
        <main className="main">
          ...
          <div className="grade-form">
            ...
            <div className="grade-actions">
              ...
              <button className="btn del" onClick={handleDeleteOne}>
                등급 삭제
              </button>
